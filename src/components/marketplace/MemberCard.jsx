...
import { MapPin, Phone, Mail, Globe, Users, ArrowRight } from 'lucide-react';
...
const levelConfig = {
  'Exclusive Category Partner': {
    badgeBg: '#FDF5E6',
    badgeColor: '#B8862B',
    borderTop: '3px solid #B8862B',
    border: '1px solid #B8862B55',
    accentColor: '#B8862B',
  },
  'Category Member': {
    badgeBg: '#E6F4F5',
    badgeColor: '#00606B',
    borderTop: '3px solid #00606B',
...
export default function MemberCard({ member, index = 0 }) {
  const cfg = levelConfig[member.level] || levelConfig['Community Member'];
    ...
    >
      ...
      <div className="p-6 border-b" style={{ borderColor: '#F3EFE7' }}>
        <div className="flex items-start justify-between gap-3 mb-2">
          ...
          <span
            className="text-[9px] font-bold tracking-[0.12em] uppercase px-2.5 py-1 flex-shrink-0 whitespace-nowrap"
            style={{ backgroundColor: cfg.badgeBg, color: cfg.badgeColor }}
          >
            {member.level === 'Exclusive Category Partner' ? 'Exclusive Partner' : member.level}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs" style={{ color: '#2C3238', opacity: 0.6 }}>
          <span className="flex items-center gap-1"><MapPin className="w-3 h-3" /> {member.market}</span>
      ...
      <div className="p-6 flex-1 space-y-4">
        ...
        <div className="space-y-2.5">
          <div>
            ...
            <p className="text-xs leading-relaxed" style={{ color: '#2C3238', opacity: 0.65 }}>{member.whoWeHelp}</p>
          </div>
          <div>
            <p className="text-[10px] font-bold tracking-[0.15em] uppercase mb-1" style={{ color: cfg.accentColor }}>Best Introduction</p>
            <p className="text-xs leading-relaxed" style={{ color: '#2C3238', opacity: 0.65 }}>{member.bestFit}</p>
          </div>
          <div>
        ...
        <div className="pt-3 border-t space-y-1.5" style={{ borderColor: '#F3EFE7' }}>
          ...
          <p className="flex items-center gap-2 text-xs" style={{ color: '#2C3238', opacity: 0.55 }}>
            <Users className="w-3 h-3 flex-shrink-0" style={{ color: cfg.accentColor }} /> Contact: {member.contact}
          </p>
        </div>
      </div>

      {/* CTAs */}
      <div className="p-5 border-t grid grid-cols-3 gap-2" style={{ borderColor: '#F3EFE7' }}>
        <Button asChild size="sm" className="text-[10px] font-semibold col-span-1"
          style={{ backgroundColor: cfg.accentColor, color: '#fff' }}>
          <Link to="/directory">View Profile</Link>
        </Button>
        <Button asChild size="sm" variant="outline" className="text-[10px] font-semibold col-span-1"
          style={{ borderColor: cfg.accentColor, color: cfg.accentColor }}>
          <Link to="/contact">Request Intro</Link>
        </Button>
        <Button asChild size="sm" variant="outline" className="text-[10px] font-semibold col-span-1"
